...
\begin{equation}
\begin{split}
&\langle A(t)\rangle = \sum_{B}\frac{e^{\beta E_B}}{Z}\bra{E_B}\bra{S} e^{-iHt}Ae^{iHt}\ket{S}\ket{E_B},\\
&Z = \sum_B e^{-\beta E_B},
\end{split}
\label{eq: canonical_ensemble}
\end{equation}

where $Z$ is the canonical partition function, $\beta$ is the inverse temperature, $\ket{E_B}$ are the microstates of the heat bath with energy $E_B$. Eq. (\ref{eq: canonical_ensemble}) can be further rewritten as\\  
